...
public class NotificationServiceImpl implements NotificationService {
    ...
    @Override
    public void sendConfirmation(AppointmentBookedEvent event) {
        if (!StringUtils.hasText(event.clientEmail())) {
            log.debug("Skipping confirmation email for appointment {}: no client email", event.appointmentId());
            return;
        }
        try {
            sendTo(event.clientEmail(), buildSubject(event), buildBody(event));
            log.info("Confirmation email sent to {} for appointment {}", event.clientEmail(), event.appointmentId());
        } catch (MessagingException | UnsupportedEncodingException e) {
            log.error("Failed to send confirmation email for appointment {}: {}", event.appointmentId(), e.getMessage());
        }
    }

    private void sendTo(String to, String subject, String htmlBody)
    ...
    private String buildSubject(AppointmentBookedEvent event) {
        return "Appointment confirmed — %s on %s".formatted(
                event.providerName(), event.scheduleStart().format(DATE_FMT));
    }

    private String buildBody(AppointmentBookedEvent event) {
        return """
                <!DOCTYPE html>
                <html lang="en">
                <head>
                ...
                event.clientName(),
                event.providerName(),
                event.providerSpecialty() != null ? event.providerSpecialty() : "—",
                event.scheduleStart().format(DATE_FMT),
                event.scheduleEnd().format(DATE_FMT),
                event.status(),
                notesRow(event.notes())
        );
    }
