...
public class ScheduleController {
    ...
    @Operation(summary = "List future available slots for a provider — used by n8n to present options to the client")
    ...
    public ResponseEntity<ScheduleResponse> create(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(scheduleService.create(providerId, request));
    }

    @PostMapping("/batch")
    @Operation(summary = "Add multiple available time slots at once for a provider")
    public ResponseEntity<List<ScheduleResponse>> createBatch(
            @PathVariable Long providerId,
            @Valid @RequestBody List<ScheduleRequest> requests) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(scheduleService.createBatch(providerId, requests));
    }
